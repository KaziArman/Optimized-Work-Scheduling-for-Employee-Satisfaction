...
def gantt_employee_availability_with_roles(*employee_ids):
    ...
    role_labels = {'Cook': 'Cook', 'Expo': 'Expo', 'Front Counter': 'Front Counter', 'Runner': 'Runner', 'Dishwasher': 'Dishwasher'}
    role_colors = {
    'Cook': '#D46F1A',
    'Expo': '#ECA61D',
    ...
    for idx, (gantt_ax, role_ax, employee_id) in enumerate(zip(axes[0], axes[1], employee_ids)):
        ...
        gantt_ax.set_xticks(list(hour_labels.keys()))
        gantt_ax.set_xticklabels([hour_labels[hour] for hour in hour_labels])
        gantt_ax.set_title(f"Employee {employee_id} Availability")
        ...
        role_ax.set_xticklabels([role_labels[role] for role in roles], rotation=0, fontweight='bold')
        role_ax.set_ylim(-0.5, 1.5)
        role_ax.set_yticks([0, 1])
        role_ax.set_yticklabels(['Not Eligible', 'Eligible'], fontweight='bold', rotation=0)
...
# Example usage:
gantt_employee_availability_with_roles(11, 12, 13)
