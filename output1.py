# -*- coding: utf-8 -*-
"""
Created on Sun Dec  1 21:28:17 2024

@author: rackl
"""

import matplotlib.pyplot as plt
import solution1 as sol
...
def plot_gantt_chart(schedule, employee_id):
    ...
    # Customize the chart
    ax.set_yticks(range(5))  # Assume a 7-day week (0 to 6)
    ax.set_yticklabels([f"{sol.weekdays[i]}" for i in range(5)])
    ax.set_xticks(list(range(9,13)) + list(range(1,10)))  # Assume 24 hours in a day (0 to 23)
    ax.set_xlabel("Hour of the Day")
    ax.set_title(f"Employee {employee_id}'s Work Schedule")
    
    # Add legend (to show roles)
    # Assuming you have the handles and labels
...
for e in sol.employees:
    ...
    start_r = emp_sch[0][2]
    last_h = -1
    plt_schedule[e].append([sol.weekdays[curr_day], start_h - 1, 0, start_r])
    for (day, hour, role) in emp_sch:
        if day == curr_day and role == start_r and hour == last_h + 1 or last_h == -1:
            last_h = hour
            continue
        plt_schedule[e][i][2] = last_h - 1
        curr_day = day
        start_r = role
        start_h = hour
        plt_schedule[e].append([sol.weekdays[curr_day], start_h - 1, 0, start_r])
