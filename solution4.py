# -*- coding: utf-8 -*-
"""
Created on Sun Dec  1 22:34:39 2024
...
z = model.addVars(employees, hours, days, days, vtype=GRB.BINARY, name="z")
...
for e in employees:
    for h in hours:
        for d1 in days:
            for d2 in days:
                if d1 != d2:
                    model.addConstr(z[e, h, d1, d2] >= x[e, r, h, d1] - x[e, r, h, d2], name=f"abs_diff_1_{e}_{h}_{d1}_{d2}")
                    model.addConstr(z[e, h, d1, d2] >= x[e, r, h, d2] - x[e, r, h, d1], name=f"abs_diff_2_{e}_{h}_{d1}_{d2}")
...
model.setObjective(gp.quicksum(z[e, h, d1, d2] for e in employees for h in hours for d1 in days for d2 in days if d1 != d2) + 10 * gp.quicksum(x[e,r,h,d] for e in range(11,13) for r in roles for h in hours for d in days), GRB.MINIMIZE)
